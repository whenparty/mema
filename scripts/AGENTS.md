...
- `README.md` — Prerequisites (Docker), cron setup, log rotation, restore procedure, monitoring

## Patterns & Decisions

- Scripts use `docker compose exec -T db` to run pg_dump/pg_restore inside the container (avoids host pg_dump version mismatch)
- B2 access via `amazon/aws-cli` Docker image (`aws_cli()` wrapper) with S3-compatible endpoint (`--endpoint-url`)
- Each script: sources `.env` from project root, validates env vars, checks for `docker`/`docker compose` commands
...
- requires: Docker, Docker Compose, running `db` service

## Testing

- Tests in `tests/scripts/` (structural/content-based, not execution)
- `tests/scripts/backup.test.ts` — script structure + .env.example B2 vars
- `tests/scripts/restore.test.ts` — script structure
