...
- `commands/handlers.ts` — `createDefaultCommandHandlers()` factory producing default `CommandHandlers` implementation with static replies.
- `commands/start.ts` — `createStartHandler(commandHandlers)` factory delegating to injected `CommandHandlers.start`.
- `commands/help.ts` — `createHelpHandler(commandHandlers)` factory delegating to injected `CommandHandlers.help`.
- `commands/stop.ts` — `createStopHandler(commandHandlers)` factory delegating to injected `CommandHandlers.stop`.
...
- `CommandReply` — Exported from `types.ts`. `(text: string) => Promise<void>`. Abstraction over platform reply mechanism, passed to command handlers.
- `CommandHandlers` — Exported from `types.ts`. `{ start, help, stop }` — injectable command handler contract. Each handler receives a `CommandReply` callback.
- `TelegramBotConfig` — Exported from `types.ts`. Configuration for `createTelegramBot`: `{ token, onMessage, isDuplicate?, commandHandlers? }`.
...
- **Command DI**: `/start`, `/help`, `/stop` delegate to injectable `CommandHandlers` (default: static placeholder text). Callers can inject custom command handlers via `TelegramBotConfig.commandHandlers` for real business logic in later tasks.
